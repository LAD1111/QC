...
interface KPICardProps {
  title: string;
  value: string;
  icon: 'revenue' | 'adCost' | 'profit' | 'margin' | 'operatingCost' | 'orders';
...
const icons = {
  revenue: (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 10v-1m0-6h.01M6 12H5m14 0h-1m-7-5h.01M7 7h.01" />
    </svg>
  ),
  adCost: (
...
const ChangeIndicator: React.FC<{ change: number, isPositiveGood: boolean }> = ({ change, isPositiveGood }) => {
    ...
    if (!isFinite(change)) {
        return <span className="text-green-600">Mới</span>;
    }
...
const KPICard: React.FC<KPICardProps> = ({ title, value, icon, comparisonValue, percentageChange, changeIsPositive = true }) => {
  return (
    <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:border-blue-500 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1">
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm text-gray-500 font-medium">{title}</p>
          <p className="text-2xl font-bold text-gray-900">{value}</p>
        </div>
        <div className="bg-gray-100 p-3 rounded-full">{icons[icon]}</div>
      </div>
       {percentageChange !== undefined && comparisonValue && (
          <div className="flex items-center text-sm mt-2">
            <ChangeIndicator change={percentageChange} isPositiveGood={changeIsPositive} />
            <span className="text-gray-500 ml-2">so với {comparisonValue}</span>
          </div>
        )}
    </div>
  );
